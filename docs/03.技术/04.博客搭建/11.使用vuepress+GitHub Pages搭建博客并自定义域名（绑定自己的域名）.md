...
![image-20241001233712291](https://fastly.jsdelivr.net/gh/xiake23/PicGo-Images/note/image-20241001233712291.png)

如配置为/blog/，那么全站的静态资源相对路径前面就会加上/blog/

![image-20241001233807406](https://fastly.jsdelivr.net/gh/xiake23/PicGo-Images/note/image-20241001233807406.png)
...
![image-20241001234857248](https://fastly.jsdelivr.net/gh/xiake23/PicGo-Images/note/image-20241001234857248.png)

编译推送成功之后，我们可以在项目的git分支上看到多了一个***gh-pages***分支，此时分支上即是我们编译后生成的静态资源文件

![image-20241001235031520](https://fastly.jsdelivr.net/gh/xiake23/PicGo-Images/note/image-20241001235031520.png)



此时打开项目的Setting-Pages配置项，即可看到咱们部署成功且可以看到相应的github Pages访问路径

![image-20241001235559354](https://fastly.jsdelivr.net/gh/xiake23/PicGo-Images/note/image-20241001235559354.png)
...
![image-20241003212739631](https://fastly.jsdelivr.net/gh/xiake23/PicGo-Images/note/image-20241003212739631.png)

第三步，在左侧栏找到 code，planning，and automation，并点击其中的 Pages选项

![image-20241003212801177](https://fastly.jsdelivr.net/gh/xiake23/PicGo-Images/note/image-20241003212801177.png)

第四步，在右侧输入要验证的域名（如 aiproducthome.top），并点击 Add domain按钮

![image-20241003212840483](https://fastly.jsdelivr.net/gh/xiake23/PicGo-Images/note/image-20241003212840483.png)

然后你会获得如下的记录，并继续按照该记录所示的步骤进行验证

![image-20241003212919214](https://fastly.jsdelivr.net/gh/xiake23/PicGo-Images/note/image-20241003212919214.png)


接记录所需验证步骤，第五步，进入你购买域名网站，并进入你的域名的管理界面，虽然不同的域名服务提供商相关的界面不同，但大体它们的组成是相同。这里以Cloudflare为例，进入网站管理处-DNS记录

![image-20241003213043130](https://fastly.jsdelivr.net/gh/xiake23/PicGo-Images/note/image-20241003213043130.png)

在记录中， 选择为 TXT 类型，名称为第四步中的 hostname，即 _github-pages 开头的内容，内容是第四步中的 code，它是一串数字和英文组成的字符串。而 TTL 值选择默认即可。

![image-20241003213146756](https://fastly.jsdelivr.net/gh/xiake23/PicGo-Images/note/image-20241003213146756.png)

当上述内容配置好后，返回第四步的验证页面，点击 `Verify`按钮。虽然理论上说的是需要 24 小时才可能验证成功，实际上不出意外，仅需要几分钟即可完成验证。

![image-20241003213310586](https://fastly.jsdelivr.net/gh/xiake23/PicGo-Images/note/image-20241003213310586.png)

![image-20241003221641382](https://fastly.jsdelivr.net/gh/xiake23/PicGo-Images/note/image-20241003221641382.png)
...
![image-20241003221216911](https://fastly.jsdelivr.net/gh/xiake23/PicGo-Images/note/image-20241003221216911.png)


### 3.3 在仓库Pages配置顶级域名

![image-20241003222025548](https://fastly.jsdelivr.net/gh/xiake23/PicGo-Images/note/image-20241003222025548.png)
...
![image-20241003214616704](https://fastly.jsdelivr.net/gh/xiake23/PicGo-Images/note/image-20241003214616704.png)

到此，在域名服务商部分需要进行的 DNS 配置就完成了，上述内容需要花费一点时间在网络的各个 DNS 服务器上进行传播，接着我们回到 Github 进行相应的配置。

如果使用A记录指向github，则会出现以下提示：

![image-20241003220754636](https://fastly.jsdelivr.net/gh/xiake23/PicGo-Images/note/image-20241003220754636.png)

### 3.6 在仓库Pages项添加自定义域名

![image-20241002011116721](https://fastly.jsdelivr.net/gh/xiake23/PicGo-Images/note/image-20241002011116721.png)

所有的任务就完成了，你可以通过顶级域名，子域名（www）或者之前的 github.io 的域名来访问，所有的内容都会跳转到自定义域名上。

### 3.7 防止重新部署导致CNAME记录绑定失败

![image-20241002011141937](https://fastly.jsdelivr.net/gh/xiake23/PicGo-Images/note/image-20241002011141937.png)
...
![image-20241002011200251](https://fastly.jsdelivr.net/gh/xiake23/PicGo-Images/note/image-20241002011200251.png)

![image-20241002123510918](https://fastly.jsdelivr.net/gh/xiake23/PicGo-Images/note/image-20241002123510918.png)
...
![image-20241002125046094](https://fastly.jsdelivr.net/gh/xiake23/PicGo-Images/note/image-20241002125046094.png)

将文件移动到public目录下

![image-20241002125626790](https://fastly.jsdelivr.net/gh/xiake23/PicGo-Images/note/image-20241002125626790.png)

重新执行部署文件后，查看静态文件资源会发现已经添加了相关的CNAME文件：

![image-20241002125803674](https://fastly.jsdelivr.net/gh/xiake23/PicGo-Images/note/image-20241002125803674.png)

![image-20241002125823282](https://fastly.jsdelivr.net/gh/xiake23/PicGo-Images/note/image-20241002125823282.png)

再次查看该仓库的Pages设置，发现已经自动绑定上我们的域名

![image-20241002011215706](https://fastly.jsdelivr.net/gh/xiake23/PicGo-Images/note/image-20241002011215706.png)
...
![image-20241003210751807](https://fastly.jsdelivr.net/gh/xiake23/PicGo-Images/note/image-20241003210751807.png)
